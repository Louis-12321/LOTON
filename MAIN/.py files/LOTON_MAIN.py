...
from PySide6.QtGui import QMovie, QPixmap, QIcon
...
Loading_Screen.update_loading_text("Loading Theme Manager...")
...
taskbar = Taskbar(
    ...
    calculator_callback=Calculator.calculator_toggle,
    notepad_callback=Notepad.notepad_toggle,
    cmd_callback=CMD.cmd_toggle,
    file_explorer_callback=File_Explorer.file_explorer_toggle,
    account_manager_callback=Account_Manager.account_manager_toggle,
    settings_callback=Settings.settings_toggle,
...
def _theme_path(theme_name, default_parts, dark_parts, light_parts):
    theme_map = {
        "Default": asset_path(*default_parts),
        "Dark": asset_path(*dark_parts),
        "Light": asset_path(*light_parts),
    }
    return theme_map.get(theme_name)


def _apply_theme_background(theme_name):
    path = _theme_path(
        theme_name,
        ("Real assets", "Lotonbackground.png"),
        ("Real assets", "Settings", "Themes", "Dark_bg.png"),
        ("Real assets", "Settings", "Themes", "White_bg.png"),
    )
    if path and Path(path).exists():
        background.setPixmap(QPixmap(path))


def _apply_theme_taskbar(theme_name):
    path = _theme_path(
        theme_name,
        ("Real assets", "Taskbar.png"),
        ("Real assets", "Settings", "Themes", "Dark_taskbar.png"),
        ("Real assets", "Settings", "Themes", "White_taskbar.png"),
    )
    if path and Path(path).exists():
        taskbar.taskbar.setPixmap(QPixmap(path))


def _apply_theme_icons(theme_name):
    icon_map = [
        (taskbar.folder, ("Real assets", "File Explorer resized.png"),
                         ("Real assets", "Settings", "Themes", "Icons", "Dark File Explorer.png"),
                         ("Real assets", "File Explorer resized.png")),
        (taskbar.calculator_button, ("Real assets", "Calculator", "Calculator button.png"),
                                    ("Real assets", "Settings", "Themes", "Icons", "Dark Calculator.png"),
                                    ("Real assets", "Calculator", "Calculator button.png")),
        (taskbar.notepad_button, ("Real assets", "Notepad", "Notepad Icon 64x64.png"),
                                 ("Real assets", "Settings", "Themes", "Icons", "Dark Notepadpng.png"),
                                 ("Real assets", "Notepad", "Notepad Icon 64x64.png")),
        (taskbar.cmd_button, ("Real assets", "CMD", "CMD icon.png"),
                             ("Real assets", "Settings", "Themes", "Icons", "Dark CMD.png"),
                             ("Real assets", "CMD", "CMD icon.png")),
        (taskbar.account_manager_button, ("Real assets", "Icons", "Account Manager.png"),
                                         ("Real assets", "Settings", "Themes", "Icons", "Dark Account Manager.png"),
                                         ("Real assets", "Icons", "Account Manager.png")),
        (taskbar.settings_button, ("Real assets", "Icons", "Resized", "Settings icon.png"),
                                  ("Real assets", "Settings", "Themes", "Icons", "Dark Settings.png"),
                                  ("Real assets", "Icons", "Resized", "Settings icon.png")),
    ]
    for label, default_parts, dark_parts, light_parts in icon_map:
        path = _theme_path(theme_name, default_parts, dark_parts, light_parts)
        if path and Path(path).exists():
            pixmap = QPixmap(path)
            if not pixmap.isNull():
    ...
    # Start button
    start_path = _theme_path(
        theme_name,
        ("Real assets", "Icons", "Resized", "Loton Start Button.png"),
        ("Real assets", "Settings", "Themes", "Loton Dark Start Button.png"),
        ("Real assets", "Icons", "Resized", "Loton Start Button.png"),
    )
    if start_path and Path(start_path).exists():
        taskbar.startbutton.setPixmap(QPixmap(start_path))


def _apply_theme_shutdown_icon(theme_name):
    path = _theme_path(
        theme_name,
        ("Real assets", "Icons", "Shutdown button.png"),
        ("Real assets", "Settings", "Themes", "Start Menu", "Shutdown Button.png"),
        ("Real assets", "Icons", "Shutdown button.png"),
    )
    if path and Path(path).exists():
        start_menu.shutdownbutton.setPixmap(QPixmap(path))


def _apply_theme_start_menu_bg(theme_name):
    path = _theme_path(
        theme_name,
        ("Real assets", "resized", "Start Menu resized.png"),
        ("Real assets", "Settings", "Themes", "Start Menu", "Start Menu resized.png"),
        ("Real assets", "resized", "Start Menu resized.png"),
    )
    if path and Path(path).exists():
        start_menu.startmenu.setPixmap(QPixmap(path))


def _apply_theme_start_menu_icons(theme_name):
    def _tab_icon(path):
        pm = QPixmap(path)
        if pm.isNull():
    ...
    def _set_btn_icon(btn, path):
        icon, isize = _tab_icon(path)
        btn.setIcon(icon)
        btn.setIconSize(isize)

    icon_map = [
        (start_menu.calculatortab, ("Real assets", "Settings", "Themes", "Icons", "Dark Calculator.png")),
        (start_menu.notepadtab, ("Real assets", "Settings", "Themes", "Icons", "Dark Notepadpng.png")),
        (start_menu.fileexplorertab, ("Real assets", "Settings", "Themes", "Icons", "Dark File Explorer.png")),
        (start_menu.commandprompttab, ("Real assets", "Settings", "Themes", "Icons", "Dark CMD.png")),
        (start_menu.accountmanagertab, ("Real assets", "Settings", "Themes", "Icons", "Dark Account Manager.png")),
        (start_menu.settingstab, ("Real assets", "Settings", "Themes", "Icons", "Dark Settings.png")),
    ]
    if theme_name == "Dark":
        for btn, dark_parts in icon_map:
            path = asset_path(*dark_parts)
            if Path(path).exists():
                _set_btn_icon(btn, path)
    else:
        default_map = [
            (start_menu.calculatortab, ("Real assets", "Calculator", "Calculator button.png")),
            (start_menu.notepadtab, ("Real assets", "Notepad", "Notepad Icon 64x64.png")),
            (start_menu.fileexplorertab, ("Real assets", "File Explorer resized.png")),
            (start_menu.commandprompttab, ("Real assets", "CMD", "CMD icon.png")),
            (start_menu.accountmanagertab, ("Real assets", "Icons", "Account Manager.png")),
            (start_menu.settingstab, ("Real assets", "Icons", "Resized", "Settings icon.png")),
        ]
        for btn, default_parts in default_map:
            path = asset_path(*default_parts)
            if Path(path).exists():
                _set_btn_icon(btn, path)


def _apply_theme_user_pic(theme_name):
    path = _theme_path(
        theme_name,
        ("Real assets", "Icons", "User pic.png"),
        ("Real assets", "Settings", "Themes", "Icons", "User.png"),
        ("Real assets", "Icons", "User pic.png"),
    )
    if path and Path(path).exists():
        pixmap = QPixmap(path)
...
Theme_Manager.register_theme_callback("wallpaper", _apply_theme_background)
Theme_Manager.register_theme_callback("taskbar", _apply_theme_taskbar)
Theme_Manager.register_theme_callback("icons", _apply_theme_icons)
Theme_Manager.register_theme_callback("shutdown", _apply_theme_shutdown_icon)
Theme_Manager.register_theme_callback("start_menu_bg", _apply_theme_start_menu_bg)
Theme_Manager.register_theme_callback("start_menu_icons", _apply_theme_start_menu_icons)
Theme_Manager.register_theme_callback("user_pic", _apply_theme_user_pic)

# Apply saved theme images at startup
_saved_theme = Theme_Manager.load_saved_theme()
_apply_theme_background(_saved_theme)
_apply_theme_taskbar(_saved_theme)
_apply_theme_icons(_saved_theme)
_apply_theme_shutdown_icon(_saved_theme)
_apply_theme_start_menu_bg(_saved_theme)
_apply_theme_start_menu_icons(_saved_theme)
_apply_theme_user_pic(_saved_theme)

# Apply saved theme stylesheets and start sync timer
