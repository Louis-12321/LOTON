...
class CMD(QWidget):
    def __init__(self):
        ...
        btn_close = QPushButton("x") #defines the button
        ...
        btn_min.clicked.connect(self.hide) #gives the button a purpose to live
    ...
    def print_banner(self):
        self.lines.append("LOTON OS [Version 1.3.5.5]")
...
#region LOGIC
    ...
    def user_input_check(self, command): #This runs everytime enter is pressed (so this do be getting reused :fire:)
        ...
        if command.lower() == "/help":
            ...
            response = "LOTON Corporation (c), All Rights Reserved."
            self.lines.append(response)  # prints the about info
...
def cmd_toggle():
    """Toggle the CMD window open/closed."""
    global _cmd_instance
    if QApplication.instance() is None:
        QApplication(sys.argv)
    if _cmd_instance is None:
        _cmd_instance = CMD()
    if _cmd_instance.isVisible():
        _cmd_instance.hide()
    else:
        _cmd_instance.show()
        _cmd_instance.raise_()
