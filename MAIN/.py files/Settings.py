...
from Theme_Manager import set_theme, install_theme_sync, load_saved_theme
...
class SettingsWindow(QWidget):
    ...
    """)
        ...
        version_label = QLabel("LOTON Version 1.3.5.5")
    ...
    """)
        ...
        # Sync theme dropdown with saved theme
        saved = load_saved_theme()
        idx = self.appearance_theme.findText(saved)
        if idx >= 0:
            self.appearance_theme.setCurrentIndex(idx)
...
def settings_toggle():
    ...
    if _settings_instance is None:
        _settings_instance = SettingsWindow()
    if _settings_instance.isVisible():
        _settings_instance.hide()
    else:
        _settings_instance.show()
        _settings_instance.raise_()
